...
// Ensures URLs always open externally — prevents relative URL being
// routed through our own API which returns "token not provided"
...
// ── Modal wrapper ─────────────────────────────────────────────
// closeOnBackdrop=false prevents accidental dismissal on task forms
function Modal({ onClose, children, maxWidth = 560, closeOnBackdrop = true }) {
  return (
...
// ── Task Detail Modal — read-only view of full task ──────────
function TaskDetailModal({ task, photos, onClose, onEdit, onDelete, onAddPhoto }) {
  const userState = (() => { try { return (JSON.parse(localStorage.getItem('user') || '{}').state || 'NSW').toLowerCase(); } catch { return 'nsw'; } })();
  const stateCode = task[`${userState}_code`] || task.nsw_code || '';

  return (
    <Modal onClose={onClose} maxWidth={520} closeOnBackdrop={false}>
      <div className="modal-header">
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            {/* Coloured subject dot */}
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: task.subject_color || 'var(--border)', flexShrink: 0 }} />
            <h2 className="modal-title" style={{ fontSize: 18 }}>{task.title}</h2>
          </div>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', paddingLeft: 18 }}>
            {task.subject_name && <span style={{ fontSize: 12, color: task.subject_color || 'var(--text-3)' }}>{task.subject_icon} {task.subject_name}</span>}
            {task.duration_minutes > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 12, color: 'var(--text-3)' }}><Clock size={11} />{task.duration_minutes}min</span>}
            {task.is_recurring === 1 && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 12, color: 'var(--text-3)' }}><RotateCcw size={11} />recurring</span>}
            {stateCode && <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--primary)', background: 'var(--primary-pale)', padding: '1px 6px', borderRadius: 4 }}>{stateCode}</span>}
          </div>
        </div>
        <button className="btn btn-ghost btn-icon" onClick={onClose}><X size={18} /></button>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {/* Resource */}
        {task.resource_title && (
          <div style={{ padding: '10px 14px', background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Resource</div>
            {task.resource_url
              ? <a href={safeUrl(task.resource_url)} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--primary)', textDecoration: 'none', fontWeight: 500 }}>
                  <ExternalLink size={13} />{task.resource_title}
                </a>
              : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-2)' }}><Link size={13} />{task.resource_title}</span>
            }
          </div>
        )}
        ...
        {task.description && (
          <div style={{ padding: '10px 14px', background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Notes</div>
        ...
        {photos.length > 0 && (
          <div>
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Photos</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {photos.map(photo => (
                <img key={photo.id} src={photo.thumbnail_url || photo.url} alt={photo.caption || ''}
                  style={{ width: 72, height: 72, borderRadius: 8, objectFit: 'cover', cursor: 'pointer' }} />
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, paddingTop: 4, borderTop: '1px solid var(--border)' }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => { onClose(); onAddPhoto(); }}>
...
// ── Task Card ─────────────────────────────────────────────────
// Compact uniform card — click to open read-only detail modal.
// Notes truncated to 50 chars to keep all cards the same height.
function TaskCard({ task, onToggle, onDelete, onEdit }) {
  ...
  const NOTE_LIMIT = 50;
  const truncatedNote = task.description && task.description.length > NOTE_LIMIT
    ? task.description.slice(0, NOTE_LIMIT) + '…'
    : task.description;
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {/* Title */}
            <div style={{ fontSize: 13, fontWeight: 500, color: task.is_completed ? 'var(--text-3)' : 'var(--text)', textDecoration: task.is_completed ? 'line-through' : 'none', wordBreak: 'break-word' }}>
              {task.title}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
              ...
              {task.resource_title && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: 'var(--primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}><ExternalLink size={10} /><span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{task.resource_title}</span></span>}
              {photos.length > 0 && <span style={{ fontSize: 11, color: 'var(--text-3)' }}>📷 {photos.length}</span>}
            </div>

            {/* Truncated notes preview */}
            {truncatedNote && (
              <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 4, fontStyle: task.description.length > NOTE_LIMIT ? 'italic' : 'normal', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {truncatedNote}
              </div>
            )}
          </div>
...
function TaskForm({ initial, kids, subjects, resources, weekStart, onSubmit, onClose, isEdit, onResourceSaved }) {
  ...
  return (
    <Modal onClose={onClose} maxWidth={580} closeOnBackdrop={false}>
      <div className="modal-header">
...
function PrintPreview({ tasks, kids, weekStart, onClose }) {
  ...
  return (
    <Modal onClose={onClose} maxWidth={820} closeOnBackdrop={false}>
