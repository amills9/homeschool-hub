...
// Ensures URLs always open externally — prevents "token not provided" errors
// when a URL was stored without https:// prefix
...
// Opens or downloads a PDF via a signed Cloudinary URL.
// Key technique: open the tab SYNCHRONOUSLY (before any await) so popup
// blockers don't interfere, then navigate it to the real URL once ready.
async function openPdf(resourceId, resourceTitle, disposition) {
  const token = localStorage.getItem('token');

  // Open blank tab immediately — this is synchronous inside the click handler
  // so browsers don't treat it as a popup. We fill it in after the fetch.
  const newTab = disposition !== 'download' ? window.open('', '_blank') : null;

  try {
    const res = await fetch(`/api/resources/${resourceId}/pdf-url`, {
    ...
    if (!res.ok) {
      ...
      alert(`Could not load PDF: ${body.error || res.statusText}`);
      return;
    }
    ...
    if (disposition === 'download') {
      // Trigger a download via a temporary <a> click
      const a = document.createElement('a');
      a.href = url;
      a.download = (resourceTitle || 'document') + '.pdf';
      a.target = '_blank';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    } else {
      // Navigate the already-open tab to the real PDF URL
      if (newTab) {
        newTab.location.href = url;
      } else {
        // Tab was blocked despite our best effort — navigate in same tab as fallback
        window.location.href = url;
      }
    }
  } catch (err) {
    if (newTab) newTab.close();
    alert('Error loading PDF. Please check your connection and try again.');
              ...
              {resource.type === 'pdf' ? (
                <>
                  <button className="btn btn-ghost btn-sm" style={{ fontSize: 11, padding: '3px 8px' }}
                    onClick={e => { e.stopPropagation(); openPdf(resource.id, resource.title, 'open'); }}>
                    <ExternalLink size={11} /> Open PDF
                  </button>
                  <button className="btn btn-ghost btn-sm" style={{ fontSize: 11, padding: '3px 8px' }}
                    onClick={e => { e.stopPropagation(); openPdf(resource.id, resource.title, 'download'); }}>
                    ⬇ Download
                  </button>
                </>
              ) : (
            ...
            <div className="input-group">
              ...
              {resource?.url && !pdfBase64 ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--border)' }}>
                  ...
                  <button type="button" className="btn btn-ghost btn-sm" style={{ fontSize: 12 }} onClick={() => openPdf(resource.id, form.title, 'download')}>⬇ Download</button>
                </div>
              ) : (
