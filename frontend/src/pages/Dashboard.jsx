...
// ── Curriculum Progress Pie Charts ───────────────────────────
// Shows DoE curriculum outcome progress per subject per child.
// "Achieved" = completed tasks linked to an outcome code.
// Click any pie to see an exploded view of done/pending outcomes.

const SUBJECT_COLORS = {
  'English': '#6C63FF', 'Mathematics': '#E76F51', 'Science': '#2D6A4F',
  'HSIE': '#219EBC', 'PDHPE': '#52B788', 'Creative Arts': '#F4A261',
  'Technology': '#8338EC', 'Languages': '#FB8500',
};
const SUBJECT_ICONS = {
  'English': '📖', 'Mathematics': '🔢', 'Science': '🔬', 'HSIE': '🌏',
  'PDHPE': '⚽', 'Creative Arts': '🎨', 'Technology': '💻', 'Languages': '🗣️',
};

// SVG donut ring — pure CSS, no library
function PieRing({ pct, color, size = 76 }) {
  const r    = (size - 14) / 2;
  const circ = 2 * Math.PI * r;
  const cx   = size / 2;
  const cy   = size / 2;
...
// Exploded detail modal shown when a pie is clicked
function OutcomeDetailModal({ subjectData, childName, userState, onClose }) {
  const stateCode = `${(userState || 'nsw').toLowerCase()}_code`;
  const { subject, outcomes, achieved, total } = subjectData;
  const color   = SUBJECT_COLORS[subject] || 'var(--primary)';
  const icon    = SUBJECT_ICONS[subject]  || '📚';
  const done    = (outcomes || []).filter(o => o.achieved);
  const pending = (outcomes || []).filter(o => !o.achieved);
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 600 }}>
        <div className="modal-header">
          <div>
            <h2 className="modal-title">{icon} {subject}</h2>
            <div style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 2 }}>
              {childName} · {achieved}/{total} outcomes achieved ({pct}%)
            </div>
          </div>
        ...
        {done.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--primary)', marginBottom: 8 }}>✅ Achieved ({done.length})</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {done.map(o => (
                <div key={o.id} style={{ padding: '8px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--primary-pale)', border: '1px solid var(--primary-light)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  {o[stateCode] && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--primary)', background: 'white', padding: '1px 6px', borderRadius: 4, flexShrink: 0, marginTop: 1 }}>{o[stateCode]}</span>}
                  <span style={{ fontSize: 12, color: 'var(--text)', lineHeight: 1.4 }}>{o.description}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {pending.length > 0 && (
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-3)', marginBottom: 8 }}>⏳ Still to come ({pending.length})</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 300, overflowY: 'auto' }}>
              {pending.map(o => (
                <div key={o.id} style={{ padding: '8px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  {o[stateCode] && <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-3)', background: 'var(--border)', padding: '1px 6px', borderRadius: 4, flexShrink: 0, marginTop: 1 }}>{o[stateCode]}</span>}
                  <span style={{ fontSize: 12, color: 'var(--text-2)', lineHeight: 1.4 }}>{o.description}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {total === 0 && (
          <div style={{ textAlign: 'center', color: 'var(--text-3)', padding: '32px 0' }}>
            No curriculum outcomes loaded for {subject} at this stage.<br />
            <span style={{ fontSize: 12 }}>Import outcomes via Admin → Curriculum</span>
          </div>
        )}
      </div>
    </div>
...
// Pie chart grid — one ring per subject, shown under child card
function SubjectPieChart({ progress, childName, userState }) {
    ...
    <>
      <div className="card" style={{ background: 'var(--surface-2)', marginTop: 16 }}>
        <div style={{ marginBottom: 12 }}>
          <h3 style={{ fontSize: 15 }}>Curriculum Progress by Subject</h3>
          <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 3 }}>
            Based on completed tasks linked to DoE outcomes · click a chart to see details
          </p>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {progress.map(s => {
            ...
            const label = s.total > 0 ? `${Math.round(pct * 100)}%` : '—';
            return (
              <div key={s.subject} onClick={() => setSelected(s)}
              ...
              >
                <div style={{ position: 'relative', width: 76, height: 76 }}>
                  <PieRing pct={pct} color={s.total > 0 ? color : 'var(--border)'} />
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: s.total > 0 ? color : 'var(--text-3)' }}>{label}</span>
                  </div>
                </div>
                <div style={{ textAlign: 'center', maxWidth: 76 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text)', lineHeight: 1.3 }}>{icon} {s.subject}</div>
                  <div style={{ fontSize: 9, color: 'var(--text-3)' }}>{s.achieved}/{s.total}</div>
                </div>
              </div>
            );
          })}
      ...
      {selected && (
        <OutcomeDetailModal subjectData={selected} childName={childName} userState={userState} onClose={() => setSelected(null)} />
      )}
    </>
  );
}
...
function ChildCard({ child, d, today, onToggleTask, defaultOpen, onOpenChange, userState }) {
    ...
    <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 20 }}>
      ...
      {open && (
        <div style={{ padding: 20 }}>
          ...
          {/* Curriculum progress pie charts — DoE outcomes by subject */}
          <SubjectPieChart progress={d.curriculumProgress || []} childName={child.name} userState={userState} />
...
export default function Dashboard() {
  ...
  const userState  = user?.state || 'NSW';
    ...
    try {
      ...
      await Promise.all(kids.map(async kid => {
        const [tasksRes, subjectsRes, progressRes] = await Promise.all([
          ...
          api.get(`/curriculum/progress/${kid.id}`).catch(() => ({ data: [] })),
        ]);
        const tasks             = Array.isArray(tasksRes.data)    ? tasksRes.data    : [];
        const subjects          = Array.isArray(subjectsRes.data) ? subjectsRes.data : [];
        const curriculumProgress = Array.isArray(progressRes.data) ? progressRes.data : [];
        ...
        allData[kid.id] = { tasks, subjects, outcomeDetails, curriculumProgress };
      }));
      setData(allData);
  ...
  return (
    <div className="animate-fade">
      ...
      {children.map((child, i) => {
        ...
        return (
          <ChildCard key={child.id} child={child} d={data[child.id]||{tasks:[],subjects:[],outcomeDetails:{},curriculumProgress:[]}} today={today} defaultOpen={defaultOpen} onToggleTask={toggleTask} onOpenChange={handleOpenChange} userState={userState}/>
        );
      })}
    </div>
  );
