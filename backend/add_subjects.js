...
for (const child of children) {
  ...
  const toAdd = DEFAULT_SUBJECTS.filter(s => existing.indexOf(s.name) === -1);
  if (toAdd.length === 0) {
    console.log('✓ ' + child.name + ' — already has all subjects');
    continue;
  }
  db.transaction(function() {
    for (const s of toAdd) insert.run(uuidv4(), child.id, s.name, s.color, s.icon, 5);
  })();
  console.log('✓ ' + child.name + ' — added: ' + toAdd.map(s => s.name).join(', '));
  total += toAdd.length;
}
console.log('Done! Added ' + total + ' subjects.');
